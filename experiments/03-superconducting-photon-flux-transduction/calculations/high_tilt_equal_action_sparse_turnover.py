#!/usr/bin/env python3
"""Sparse equal-dark-action capture probe for the high-tilt turnover.

Use exact same-environment nonlocal actions at delta=.15,.16,.18 and restore
B_target=37.61 with the exact electrical similarity.  The physical 14-um / 20-ps
thermal pulse is unchanged.  These points deliberately span the regime where the
compensated cold phase clock drops from ~7.3 to ~4.7 GHz.
...
B_TARGET=37.61
B_DISS={.150:11.16332163,.160:9.90315950,.180:7.58847202}
AREA_GRID={
 .150:(190.,210.,230.,250.,270.),
 .160:(205.,230.,255.,280.,305.),
    ...
    try:
        ...
        for A in AREA_GRID[delta]:
            o=nf.run_case(model,14.,R=R,alpha=ALPHA,ntraj=2048,dt_ps=.125,
                          seed=int(1500000+1000*delta),area_um2=A,rise_ps=20.)
            ...
            print(msg); print(f'::notice title=Experiment 03 sparse turnover capture::{msg}')
    finally:
