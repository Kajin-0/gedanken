#!/usr/bin/env python3
"""Equal-dark-action high-tilt capture frontier at delta=.12,.13,.14.

Exact same-environment nonlocal actions are taken from the validated continuation.
For each tilt restore B_target=37.61 using the exact electrical similarity
C->r^2 C, R->R/r, alpha fixed. The physical 14-um / 20-ps graphene thermal
...
B_TARGET=37.61
B_DISS={.120:15.4275357,.130:13.9174700,.140:12.4990329}
AREA_GRID={
 .120:(148.,156.,164.,172.,180.),
 .130:(164.,174.,184.,194.,204.),
 .140:(178.,190.,202.,214.,226.),
}
    ...
    try:
        ...
        for A in AREA_GRID[delta]:
            o=nf.run_case(model,14.,R=R,alpha=ALPHA,ntraj=2048,dt_ps=.125,
                          seed=int(1200000+1000*delta),area_um2=A,rise_ps=20.)
            ...
            print(msg); print(f'::notice title=Experiment 03 high-tilt extension4::{msg}')
    finally:
