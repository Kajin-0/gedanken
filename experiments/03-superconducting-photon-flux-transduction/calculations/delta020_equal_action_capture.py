#!/usr/bin/env python3
"""Equal-action capture at delta=.20 near the finite-T dark crossover boundary.

The exact base same-environment action is B0=5.52063400.  Restore B_target=37.61
with the exact electrical similarity, leaving the physical 14-um / 20-ps thermal
pulse unchanged.

This is a symmetrized-FDT TWA capture screen only.  At delta=.20 the simple
20-mK finite-temperature dark diagnostic is already close to crossover, so even
a favorable capture result does not authorize this point as a physical design.
"""
...
B0=5.52063400; BT=37.61
C0=215e-15; R0=80.; L0=111.5e-12; ALPHA=.90

def main():
    delta=.20; r=BT/B0; C=C0*r*r; R=R0/r
    ...
    try:
        ...
        print(f'delta=.200 r={r:.9f} C={C*1e15:.3f}fF R={R:.4f}ohm fold={fold:.6f}K '
              f'fc={wc/(2*math.pi)*1e-9:.5f}GHz Btarget={BT:.5f}')
        for A in (360.,420.,460.,500.,540.):
            o=nf.run_case(model,14.,R=R,alpha=ALPHA,ntraj=2048,dt_ps=.125,seed=200200,
            ...
            print(msg); print(f'::notice title=Experiment 03 delta020 equal-action capture::{msg}')
    finally:
