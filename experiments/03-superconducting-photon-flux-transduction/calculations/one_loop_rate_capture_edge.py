#!/usr/bin/env python3
"""2-ns capture screen at the last Gaussian-valid one-loop dark-rate tilts.

These points extend `one_loop_rate_constrained_capture.py` to delta=.211 and .212,
using the self-consistent electrical scales that satisfy

    Gamma_1loop(T0=20 mK) = 1e-6 /s.

The .213 rate root is intentionally excluded: its T0/Tx=0.9615 classification is
already in the soft-mode-provisional region.  This script therefore compares
only points that remain accepted under the ordinary Gaussian periodic-instanton
prefactor treatment.
"""
...
AREA_GRID={
    .2110:(465.,480.,495.,510.,525.),
    .2120:(470.,490.,510.,530.,550.),
}
...
def main():
    ...
    try:
        ...
        hdr=(f'delta={d:.4f} r_rate={r:.9f} C={C*1e15:.3f}fF R={R:.6f}ohm '
             ...
             f'constraint=Gamma1loop_20mK=1e-6/s status=GAUSSIAN_ACCEPTED')
        print(hdr); print(f'::notice title=Experiment 03 edge one-loop design::{hdr}')
        for A in AREA_GRID[d]:
            o=nf.run_case(model,14.,R=R,alpha=ALPHA,ntraj=1024,dt_ps=.125,tpost_ns=2.0,
                          seed=4210000+int(round(10000*d)),area_um2=A,rise_ps=20.)
            ...
            print(msg); print(f'::notice title=Experiment 03 edge one-loop capture::{msg}')
        print('PASS')
    finally:
