...
\label{eq:factor}
\end{equation}
to retained narrowband order, where $H_A$ maps selected source inputs to gravitational outputs, $H_B$ maps gravitational inputs to selected receiver outputs, and $P_g$ is the normalized free-space gravitational propagation operator.  If
\begin{equation}
\|P_g(\omega_0)\|_{\op}^2\le\eta_{\max},
\end{equation}
then contractivity of the passive endpoint scattering blocks and Eq.~\eqref{eq:h2cut} give two cuts,
\begin{equation}
\Gamma_{\rm coh}
\le
\eta_{\max}\Tr(K_{g,A}^\dagger K_{g,A}),
\end{equation}
\begin{equation}
\Gamma_{\rm coh}
\le
\eta_{\max}\Tr(K_{g,B}^\dagger K_{g,B}).
\end{equation}
Hence
\begin{equation}
\boxed{
...
\end{equation}
obeys the operator inequality $0\le P_i(\tau)\le I$.  The monotone strong limit therefore satisfies $0\le P_i\le I$.  Section~\ref{sec:resource} shows that the gravitational matter resource itself implies $\Tr(K_g^\dagger K_g)<\infty$, making $K_g$ Hilbert--Schmidt and the operator-valued $H_2$ norm finite.  This covers countably infinite \emph{bounded-port} modal sectors; arbitrary unbounded boundary-control PDE ports require separate admissibility analysis \cite{BarasBrockett1975,Opmeer2013}.
