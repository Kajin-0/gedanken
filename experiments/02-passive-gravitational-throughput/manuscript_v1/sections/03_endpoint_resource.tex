...
\label{eq:qn}
\end{equation}
For a harmonic mode of frequency $\omega_n$, the standard quadrupole power formula gives the gravitational energy-decay rate
\begin{equation}
\boxed{
\kappa_{g,n}
...
If the retained modes satisfy $\omega_n\le\Omega$, Eqs.~\eqref{eq:kappan} and \eqref{eq:qsum} imply
\begin{equation}
\boxed{
\sum_n\kappa_{g,n}
\le
...
For a narrow carrier band, $\Omega=\omega_0[1+O(B/\omega_0)]$, so
\begin{equation}
\Tr(K_g^\dagger K_g)
\lesssim
