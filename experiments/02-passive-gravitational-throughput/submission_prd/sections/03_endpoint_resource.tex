...
&=\frac{20}{3}I_2 .
\end{align}
No orthogonality among the different $g^{ij}$ fields is required. For a complete displacement basis the corresponding Parseval relation saturates this unweighted projection sum; truncating the retained modal set can only decrease it. The theorem does not require the retained carrier-scale subset to saturate the complete-basis value: the retained fraction depends on the body's elastic spectrum and geometry, and no tightness claim for the $20/3$ coefficient within a particular operating band is made here. Therefore
\begin{equation}
\boxed{
