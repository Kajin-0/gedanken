...
\label{eq:crossH}
\end{equation}
The strict-properness condition has a direct physical meaning in the application below: a selected local drive first excites endpoint dynamical degrees of freedom before gravitational radiation is emitted, and a separated receiver is driven by the retarded TT field. There is no instantaneous selected-input-to-selected-output bypass. Models containing a direct feedthrough term lie outside the present $H_2$ statement.
...
\label{eq:h2cut}
\end{equation}
This is generic passive-system mathematics, not a gravitational result. It is conceptually related to classical gain--bandwidth restrictions, but its $H_2$ selected-port form is not the same functional as Fano's reflection-coefficient matching integrals \cite{Fano1950}.
...
\label{eq:factor}
\end{equation}
to retained carrier-frozen narrowband order. Here $H_A$ maps the selected source inputs to the complete retained set of gravitational output ports, $H_B$ maps the complete retained gravitational input set to selected receiver outputs, and $P_g$ is the normalized free-space gravitational propagation operator evaluated at the carrier. Nongravitational loss ports remain in the total operator $K$; such losses can make the final ceiling very loose, but cannot increase the allowed transfer. The strict inequalities below therefore apply to the carrier-frozen retained model; replacement of the exact frequency-dependent propagator by $P_g(\omega_0)$ is part of the separately declared narrowband approximation. Let
\begin{equation}
