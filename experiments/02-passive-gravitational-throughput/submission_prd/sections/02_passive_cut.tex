...
The same Gramian argument extends directly to a separable countably infinite modal Hilbert space when the chosen local-in-time realization has bounded Markov port operators and the retained gravitational observation is Hilbert--Schmidt. Here ``Markov'' refers to the state realization, not necessarily to a reduced observable. A nonlocal memory kernel produced by projecting out passive harmonic degrees of freedom can therefore remain inside the logic after those degrees of freedom are restored to an enlarged state space, provided the enlarged dynamics is well posed and passive and the gravitational observation satisfies the required trace condition \cite{Stella2014}. Conversely, non-Markovianity by itself is not an escape from the passive cut.

The present proof does not establish that every hereditary or continuum reduced model admits such an enlargement with the needed admissibility and Hilbert--Schmidt properties. Likewise, unbounded control or observation operators in distributed-parameter models require system-node/admissibility analysis rather than the bounded-operator calculation above. Broad classes of beam and Maxwell-type PDEs are nevertheless known to admit scattering-passive well-posed realizations with unbounded internal operators \cite{StaffansWeiss2012}; thus ``unbounded PDE'' is a proof obligation, not a synonym for nonpassive or non-Markovian physics.