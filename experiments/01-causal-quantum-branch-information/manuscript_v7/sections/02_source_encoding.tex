...
Each spoke couples only to its controller field at the same spacetime point through the local eigenstrain density
\begin{equation}
\boxed{
\mathcal E_a(x,t)
=\frac12EA
...
\epsilon_as
\left[
u f_q'-\lambda\chi X_C(t-x/v_c)\right].
\end{equation}
Therefore
\begin{equation}
\boxed{
...
\label{eq:uphi}
\end{equation}
For a narrowband branch-conditioned controller component, define the normalized overlap weight $w(x)\propto\chi(x)f_q'(x)$ and the causal form factor
\begin{equation}
\boxed{
F_c(\omega)=
