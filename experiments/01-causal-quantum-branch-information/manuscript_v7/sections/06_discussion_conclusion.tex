...
The spoke, hub, and eigenstrain controller form a closed effective linear-elastic source to the retained order. A microscopic covariant hyperelastic completion is outside the present scope.
...
\end{equation}
to the working perturbative order. An exact controller-empty handoff then permits the complete coherent branch information to be compressed into one virtual bosonic difference mode. This supplies the physical channel input required by the subsequent bosonic analysis rather than assuming it at the outset. Precursor radiation emitted during the encoder remains part of the complete waveform and is not reset at handoff.
