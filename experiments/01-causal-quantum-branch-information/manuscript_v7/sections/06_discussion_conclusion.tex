...
\paragraph{Branch-distance terminology.}
The quantities $N_\Delta$ and its port-resolved components are squared coherent-state displacement distances between the two logical branches. They are not themselves information measures such as entropy, mutual information, channel capacity, or entanglement. Operational quantum capability is assessed only after source noise, propagation loss, receiver noise, and readout are included through the channel criteria below.
...
\paragraph{Macroscopic source coherence.}
The link coefficient is conditional on successful preparation and preservation of the modeled source--reference coherence through the relevant encoding and emission interval. In particular, the kilogram-scale benchmark does not include a device-specific environmental decoherence calculation for realizing that coherent mechanical source. A physical implementation would require the source coherence time to exceed the protocol time after all thermal, vibrational, material, controller, and measurement-induced decoherence is included. That practical requirement may be more restrictive than the already tiny gravitational transfer coefficient; it is deliberately not presented here as solved experimental engineering.
...
\paragraph{Infrared and asymptotic scope.}
The equal-Poincar\'e-charge condition is used only within the cited first-order gravitational-splitting construction. We do not claim that the ten Poincar\'e generators exhaust all infrared or asymptotic gravitational data beyond that perturbative setting, nor that soft or memory sectors are absent in a more complete asymptotically flat treatment. Any extension in which such sectors carry branch-dependent records would have to include them as additional gravitational output degrees of freedom in the same global branch-distance accounting.
...
\end{equation}
to the working perturbative order. A controller-clear handoff then permits the complete coherent branch dependence to be compressed into one virtual bosonic difference mode. This supplies the physical channel input required by the subsequent bosonic analysis rather than assuming it at the outset. Precursor radiation emitted during the encoder remains part of the complete waveform and is not reset at handoff.
